...
//		}
		...
		if (lastNo == null) { // 처음이라면
			memberDao.increaseMeberPoint(board_writer, 50);
		} // 포인트 50점 부여
			...
			if (seconds > 300) { // 시간차가 5분(300초)보다 크다면
				memberDao.increaseMeberPoint(board_writer, 10);
			} // 포인트 10점 부여}
		}
...
//	@RequestMapping("/list")
//	public String list(Model model, HttpSession session, BoardDto boardDto, @RequestParam(required = false) String type,
...
//삭제
	...
	public String delete(@RequestParam int board_no, HttpSession session, @ModelAttribute BoardDto inputDto) {
		BoardDto boardDto = boardDao.detail(board_no);
